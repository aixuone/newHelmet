layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("build/js/newsList.json", function(data){
		//console.log("---------------------------"+data[1].newsName)
		var newArray = [];
		//单击首页“待审核文章”加载的信息
...
//		点击编辑按钮，将此行信息存进数组td
		...
		for(var i=1;i<=6;i++){
			if(i!=5){
				...
				//为每个数据加一个input框
				td.eq(i).empty();
				td.eq(i).append('<input type="text" class="input_add" value='+arr[i]+' />');
			}
		}
		//添加一个按钮
		$(this).parent().prepend('<a class="layui-btn layui-btn-xs news_update"><i class="fa fa-edit"></i> 修改</a>');
		$(this).remove();
	})
	$("body").on("click",".news_update",function(){//修改
		//点击修改，收录所有的数据
		var arr=new Array(7);
		var td=$(this).parent().siblings("td");
		for(var i=1;i<=6;i++){
			if(i!=5){
				arr[i]=td.eq(i).children("input").val();
				td.eq(i).empty();
				td.eq(i).append(arr[i]);
			}
		}
		layer.alert('您好，数据已修改',{icon:6, title:'文章编辑'});
		//修改按钮名称
		$(this).parent().prepend('<a class="layui-btn layui-btn-xs news_edit"><i class="fa fa-edit"></i> 编辑</a>');
		$(this).remove();
	})
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td><div class="layui-table-cell laytable-cell-1-pic"> <div class="layui-upload"> <button class="layui-btn layui-btn-xs test_upload" id="upload">上传图片</button> <input class="layui-upload-file" type="file" name="file"> <div class="layui-upload-list"> <!--此处加layui-hide可隐藏--> <img class="layui-upload-img" id="demo1"> <p id="demoText"></p> </div> </div> </div></td>'
			    	+'<td>'
					+  '<a class="layui-btn layui-btn-xs news_edit"><i class="fa fa-edit"></i> 编辑</a>'
					+  '<a class="layui-btn layui-btn-normal layui-btn-xs news_collect"><i class="fa fa-star"></i> 收藏</a>'
					+  '<a class="layui-btn layui-btn-danger layui-btn-xs news_del" data-id="'+data[i].newsId+'"><i class="fa fa-trash"></i> 删除</a>'
			        +'</td>'
			    	+'</tr>';
				}
			}else{
